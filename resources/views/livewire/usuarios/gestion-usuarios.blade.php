                    ...
                    <td class="p-4 flex justify-center space-x-2">
                        @if(!$user->hasRole('SUPER ADMIN'))
                            ...
                            @can('usuarios.eliminar')
                                <button wire:click="confirmarEliminar({{ $user->id }})" class="p-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">
                                    <x-heroicon-o-trash class="w-5 h-5" />
                                </button>
                            @endcan
                        @else
                            <span class="px-3 py-1 bg-yellow-900/50 text-yellow-300 rounded-full text-xs font-bold border border-yellow-800 uppercase tracking-wider">
                                Protegido
                            </span>
                        @endif
                    </td>
    ...
    @if($isModalOpen)
        <div class="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-50 p-4">
            <div class="bg-gray-800 rounded-lg w-full max-w-lg border border-gray-700 shadow-2xl overflow-hidden">
                <div class="{{ $user_id ? 'bg-green-600' : 'bg-blue-600' }} p-4">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex justify-center">
                        <div class="w-24 h-24 bg-gray-700 rounded-full overflow-hidden border-2 border-gray-600 relative">
                            ...
                            @else
                                <div class="flex items-center justify-center h-full text-gray-400 text-xs">Foto</div>
                            @endif
                        </div>
                    </div>

                    <input type="file" wire:model="image" class="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-gray-700 file:text-white hover:file:bg-gray-600 cursor-pointer">

                    <div class="relative">
                        <span class="absolute left-3 top-3 text-gray-500">👤</span>
                        <input wire:model="name" type="text" placeholder="Nombre del usuario"
                            class="w-full pl-10 p-2.5 bg-gray-700 text-white rounded-lg border {{ $errors->has('name') ? 'border-red-500' : 'border-gray-600' }}">

                        <x-error-message for="name" />
                    </div>

                    <div class="relative">
                        <span class="absolute left-3 top-3 text-gray-500">✉️</span>
                        <input wire:model="email" type="email" placeholder="correo@ejemplo.com"
                            class="w-full pl-10 p-2.5 bg-gray-700 text-white rounded-lg border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-600' }}">
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-3 text-gray-500">🛡️</span>
                        <select wire:model="role" class="w-full pl-10 p-2.5 bg-gray-700 text-white rounded-lg border {{ $errors->has('role') ? 'border-red-500' : 'border-gray-600' }}">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="relative">
                            ...
                            <input wire:model="password" type="password" placeholder="Contraseña"
                                class="w-full pl-10 p-2.5 bg-gray-700 text-white rounded-lg border {{ $errors->has('password') ? 'border-red-500' : 'border-gray-600' }}">
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-3 text-gray-500">🛡️</span>
                            <input wire:model="password_confirmation" type="password" placeholder="Confirmar"
                                class="w-full pl-10 p-2.5 bg-gray-700 text-white rounded-lg border {{ $errors->has('password_confirmation') ? 'border-red-500' : 'border-gray-600' }}">
                ...
                <div class="p-6 bg-gray-900 border-t border-gray-700 flex justify-end space-x-3">
                    <button wire:click="$set('isModalOpen', false)" class="px-4 py-2 text-gray-400 hover:text-white transition">
                        Cancelar
                    </button>

                    <button wire:click="guardar"
                        class="{{ $user_id ? 'bg-green-600 hover:bg-green-500' : 'bg-blue-600 hover:bg-blue-500' }} text-white px-6 py-2 rounded-lg transition shadow-lg">
